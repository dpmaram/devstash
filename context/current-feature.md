...
- 2026-05-10 EDT - Fixed UI review findings on `main`: added `usePathname()`-based active link highlighting (bg-white/[0.08] + aria-current) to all sidebar nav links in `DashboardChrome.tsx` (item types, collections, view-all), and added a "Sign up with GitHub" button to `RegisterForm.tsx` matching the sign-in page pattern with loading/disabled state. Verified with `npx tsc --noEmit` and `npm run lint`.
